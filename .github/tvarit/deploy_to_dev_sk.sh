...
sed -i "s#<AWS_ACCESS_KEY/>#${AWS_ACCESS_KEY}#g" instance.sh
sed -i "s#<AWS_SECRET_KEY/>#${AWS_SECRET_KEY}#g" instance.sh
...
aws lightsail attach-static-ip  --static-ip-name grafana-${PREFIX} --instance-name grafana-${PREFIX}
